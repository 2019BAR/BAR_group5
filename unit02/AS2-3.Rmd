...
```{r}
summary(A)
#Married,  MetroAreaCode, Education, EmploymentStatus, Industry 
```
...
```{r}
B = tapply(is.na(A$MetroAreaCode), A$State, mean) %>% sort
nrow(B)
#51
```
...
```{r}
C = tapply(A$MetroAreaCode, A$State, mean) %>% sort
nrow(C)
#3
```
...
```{r}
names(merge(A, CountryMap, by.x="CountryOfBirthCode", by.y="Code", all.x=TRUE))
```
