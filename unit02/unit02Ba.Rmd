...
##### 分類統計功能 `tapply()`
```{r}
tapply(WHO$Over60, WHO$Region, mean)#各區域超過60歲平均人口數
```
mean of over 15 by region



```{r}
tapply(WHO$LiteracyRate, WHO$Region, min)#各區域識字率最低的人數
```

```{r}
tapply(WHO$LiteracyRate, WHO$Region, min, na.rm=TRUE)#na.rm=排除NA
```
...
```{r}
range(USDA$Sodium, na.rm=T)#range = 最小到最大
```
...
hist(USDA$VitaminC, xlab = "Vitamin C (mg)", 
     main = "Histogram of Vitamin C", xlim = c(0,100))#xlim = x軸0~100

# Specify breaks of histogram
hist(USDA$VitaminC, xlab = "Vitamin C (mg)", 
     main = "Histogram of Vitamin C", xlim = c(0,100), breaks=100)#breaks = 切幾格
