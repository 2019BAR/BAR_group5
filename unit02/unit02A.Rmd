...
```{r}
df[df$height > 180 , "name"] # 身高高於180的人
```

```{r}
subset(df, height > 170 & !isMale)$name # 身高高於170的女生
```

```{r}
mean(df$height[df$isMale]) # 男生平均身高
```

```{r}
df$height[!df$isMale] %>% mean # 女生平均身高
```

```{r}
sum( subset(df, !isMale)$noBuy ) # 女生總共買了多少次
```

```{r}
subset(df, skin_color == "white" & !isMale ) %>% nrow # 計算白種女生的人數
```

```{r}
sum(df$skin_color == "white" & !df$isMale ) # 白種女生有幾人
```
