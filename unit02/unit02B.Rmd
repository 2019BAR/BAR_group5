...
```{r}
table(USDA$HighSodium)#列出高鹽數量
```

```{r}
table(USDA$HighSodium, USDA$HighFat)#列出高鹽和高脂肪數量
```

```{r}
tapply(USDA$Iron, USDA$HighProtein, mean, na.rm=TRUE)#鐵質和高蛋白的平均值，移除遺漏值
```

```{r}
tapply(USDA$VitaminC, USDA$HighCarbs, max, na.rm=TRUE)#維他命c和高碳水的最高值，移除遺漏值
```

```{r}
tapply(USDA$VitaminC, USDA$HighCarbs, summary, na.rm=TRUE)#檢視維他命c和高碳水，移除遺漏值
```
<br>
