...
```{r}
#建模型(Training)
glm3 = glm(PoorCare ~ .,TR[,-1], family=binomial)
...
```{r}
#預測機率分佈(DPP)
data.frame(y=factor(TR$PoorCare), pred3=pred3) %>% 
...
data.frame(y=factor(TS$PoorCare), pred4=pred4) %>% 
...
```{r}
#模型準確性指標
sapply(list(Train=cmx3, Test=cmx4), Accuracy)
```
...
```{r}
# A1: 根據兩個模型的AUC數值來看
# 可以發現多個預測變數模型的trAUC和tsAUC都比兩個預測變數模型的大，所以推論多個預測變數的模型比較準
# A2: 因為新的模型考慮了更多預測變數
```
